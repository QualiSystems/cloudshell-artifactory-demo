...
def install_package(host, user, password, local_path, remote_path):
    ...
    stdout.readlines()
    stdin, stdout, sterr = ssh.exec_command("python -m pip install django==1.10.3")
    stdout.readlines()
    stdin, stdout, sterr = ssh.exec_command("tar -xvf {0} > filename".format(remote_path))
    stdout.readlines()
    stdin, stdout, sterr = ssh.exec_command("tar -xvf cloudshell-artifactory-demo.tar -C /root > filename")
    stdout.readlines()
    stdin, stdout, sterr = ssh.exec_command("nohup python ./cloudshell-artifactory-demo/demowebapp/manage.py runserver 0.0.0.0:8080 >/tmp/out.txt 2>&1 &")
...
class Artifactory:
    ...
    def download(self, buildName, buildNumber):
        safe_name = base64.urlsafe_b64encode(buildName)
        file_name = "binaries_for_{0}_{1}.tar".format(safe_name, buildNumber)
